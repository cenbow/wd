package com.okwei.company.util;
...
public class LocalRedisUtil {
	...
	private static Log logger = LogFactory.getLog(LocalRedisUtil.class);

	// Redis服务器IP
	private static String ADDR_ARRAY = PropertyFactory.getPropertyValue("redis", "redis.ip");

	// Redis的端口号
	private static int PORT = Integer.parseInt(PropertyFactory.getPropertyValue("redis", "redis.2port"));
