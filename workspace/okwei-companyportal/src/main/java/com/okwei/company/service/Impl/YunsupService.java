...
public class YunsupService extends BaseService implements IYunsupService {
			...
			if (pcList != null && pcList.size() > 0) {
				for (PProductClass pc : pcList) {
					...
					if (parentId == 1) {
						...
						case "衣":
							bkv.setCssclass("spn_red1");
							break;
						case "食":
							bkv.setCssclass("spn_red2");
							break;
						case "住":
							bkv.setCssclass("spn_red3");
							break;
						case "行":
							bkv.setCssclass("spn_red4");
							break;
						case "用":
							bkv.setCssclass("spn_red5");
							break;
						case "玩":
							bkv.setCssclass("spn_red6");
							break;
						case "学":
							bkv.setCssclass("spn_red7");
							break;
						default:
							break;
						}
	...
	public List<PProductClass> getHotClasstwoList() throws Exception {
		...
		if (list == null) {
			...
			String sql = "SELECT classID,parentID,className,step " + "FROM P_ProductClass WHERE classid IN (SELECT classID FROM (SELECT classID,COUNT(1) AS ct FROM P_Products GROUP BY classID ORDER BY ct DESC LIMIT 20) AS temp)";
	...
	public List<PBrand> getHotBrandList() throws Exception {
		...
		if (list == null) {
			...
			String sql = "SELECT brandID,brandName FROM P_Brand " + "WHERE status = 1 and  brandID IN (SELECT brandID FROM (SELECT brandID,COUNT(1) AS ct FROM P_Products GROUP BY brandID ORDER BY ct DESC LIMIT 20) AS temp)";
	...
	public PageResult<YunSupVo> getUBatchSupplyerList(Limit buildLimit, YunSupParam queryparam, Long weiId) throws Exception {
		...
		if (yunsupPage == null || yunsupPage.getList() == null || yunsupPage.getList().size() <= 0) {
			...
			if (yunObjList != null && yunObjList.size() > 0) {
				int totalCount = (int) yunsupDao.countBySql("SELECT count(1) FROM (select sc.productId FROM APP_PScoreStatics sc JOIN U_YunSupplier yun ON yun.weiid=sc.SupplyWeiid AND yun.Status=4 LEFT JOIN P_Products p ON p.ProductID = sc.productID " + sb.toString()
						+ " AND p.State = 1 GROUP BY supplyweiid) t", params);
					...
					if (yun.getWeiId() != null && yun.getWeiId() > 0) {
						...
						if (weiSeller != null) {
							...
							if (yunsup != null) {
								...
								// 主营
								String sql = "SELECT B.ClassName FROM (SELECT CategoryID FROM U_SupplierBusCategory WHERE WeiID = ?) AS A " + "LEFT JOIN P_ProductClass AS B ON A.CategoryID=B.ClassID";
