...
<div class="w bg-w">
		...
		<div class="mar1200 lin40 f12 h40 heanew ">
			...
			<div class="fl pr pl5 pt10 pr5 pb5" id="Yijian">
				...
				<div class="promotion_txt none" id="Fonts">
					<textarea id="zxxTestArea">微店网打通微商城，让${headinfo.userCount }万微店主为您推销产品。网络营销不在乎平台新老，有流量、投入产出成正比，就上！<%=joindomain%><c:if test="${user.isIndex<1&&user.weiID>0 }">/supplier/apply?w=${user.weiID }</c:if></textarea>
			...
			<div class="fr ml10 wzdban pr" id="WzDao">
				...
				<div class="reabdh pa none" id="Dhang">
					<p>
						<a href="http://www.<%=okweidomain %>/list/list">云端产品库</a>
					</p>
					<p>
						<a href="<%=companydomain%>/wholesale/index">批发市场</a>
					</p>
					<p>
						<a href="<%=companydomain%>/yun/list?c=0&type=0">工厂</a>
					</p>
					<p>
			...
			<c:choose>
				<c:when test="${user.weiID>0 }">
					<!-- 已登录 -->
					<p class="fr imgrs">
						<a href="http://www.${user.weiID }.okwei.com">${user.weiName }</a> <a href="javascript:SignOut();" class="c6">退出</a> <a href="<%=mydomain%>">后台管理</a><a href="<%=cartdomain%>/shopCartMgt/list">购物车（<span class="tb cR">${user.cartCount }</span>） 
					</p>
				</c:when>
				<c:otherwise>
					<!-- 未登录 -->
					<p class="fr imgrs">
						<a href="javascript:goLogin();">请登录</a> <a href="<%=portdomain%>/RegistNew.aspx">免费开微店</a><a href="<%=cartdomain%>/shopCartMgt/list">购物车（<span class="tb cR">${user.cartCount }</span>） 
					</p>
				</c:otherwise>
			</c:choose>
	...
	<div class="w dht">
		<div class="mar1200 dht">
			<ul class="banone fl ">
				<li class="pr nows" id="nav_all"><a href="javascript:;" class="nows">全部分类 <i class="nav_down"></i></a>
											...
											<p>
												<c:forEach var="menutwo" items="${menuone.list }" varStatus="vs">
													<c:if test="${vs.index<6 }">
														<a target="_blank" href="http://www.<%=okweidomain %>/list/list?mid=${menutwo.id }">${menutwo.name }</a>
													</c:if>
												</c:forEach>
											</p>
										</li>
											...
											<li class="subItem">
												...
												<p class="subItem-cat">
													<c:forEach var="menuthree" items="${menutwo.list }">
														<a target="_blank" href="http://www.<%=okweidomain %>/list/list?sid=${menuthree.id }">${menuthree.name }</a>
													</c:forEach>
												</p>
											</li>
...
<li name="nav_li"><a href="<%=companydomain %>/wholesale/index" <c:if test="${mtype=='0'}">class="now_2 ficnews"</c:if>>批发市场</a></li>
<li name="nav_li"><a href="<%=companydomain %>/yun/list" <c:if test="${mtype=='1'}">class="now_2 ficnews"</c:if>>工厂</a></li>
...
<ul class="bantwo fr">
	<li><a href="http://www.<%=okweidomain %>/list/list">云端产品库</a></li>
	<li><a href="<%=joindomain%>/supplier/apply">供应商进驻</a></li>
</ul>
</div>
</div>
</div>
<!-- 菜单效果 -->
<script src="/statics/js/jquery.tmailsider.js"></script>
<script type="text/javascript">
	(function($) {
