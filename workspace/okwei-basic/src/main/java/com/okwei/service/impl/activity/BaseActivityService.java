...
public class BaseActivityService implements IBaseActivityService {
	...
	public SaleAreaModel getSaleAreaModel(Long weiid) {
		...
		if (flashSaleList != null && flashSaleList.size() > 0) {
			...
			if (list != null && list.getList() != null && list.getList().size() > 0) {
				...
				String apiV3= ResourceBundle.getBundle("domain").getString("currentApiV3Domain");
				if (price == (int) price) {
					//result.setTagImageUrl("http://" + AppSettingUtil.getSingleValue("currentApiV3Domain") + "/VersionThree/GenerateImg?price=" + (int) price);
					result.setTagImageUrl(apiV3+"/VersionThree/GenerateImg?price=" + (int) price);
				} else {
					//result.setTagImageUrl("http://" + AppSettingUtil.getSingleValue("currentApiV3Domain") + "/VersionThree/GenerateImg?price=" + price);
					result.setTagImageUrl(apiV3+"/VersionThree/GenerateImg?price=" +  price);
				}
			}
		}
		return result;
	...
	public AActSupplier getAActSupplier(Long actID,Long weiid){
		return baseDAO.getNotUniqueResultByHql(" from AActSupplier a where a.actId=? and a.supplyWeiid=? ", actID,weiid);
	}
	...
	public ReturnModel editAActivityProducts(Long weiid,Double amount, List<AActivityProducts> list){
		if(amount>0&&list!=null&&list.size()>0){
			AActSupplier supplier=baseDAO.getNotUniqueResultByHql("from AActSupplier a where a.actId=? and a.supplyWeiid=? ", list.get(0).getActId(),weiid);
			if(supplier!=null){
				supplier.setAmount(amount);
				baseDAO.update(supplier);
			}else {
				supplier =new AActSupplier();
				supplier.setActId(list.get(0).getActId());
				supplier.setSupplyWeiid(weiid);
				supplier.setAmount(amount);
				supplier.setCreateTime(new Date());
				baseDAO.save(supplier);
			}
		}
		return editAActivityProducts( weiid,  list);
	}
	...
	public ReturnModel editAActivityProducts(Long weiid, List<AActivityProducts> list) {
		...
		if (list != null && list.size() > 0) {
			...
			if(activity.getType()==null||activity.getType()<=0){//全返活动不限制数量
				if ((count + list.size()) > 20 && weiid!=20162055&&weiid!=9991) {
					rqModel.setStatu(ReturnStatus.SystemError);
			...
			for (AActivityProducts aa : list) {
				...
				if(aa.getTraddingPrice()!=null&&aa.getTraddingPrice()>0&&activity.getType()!=null&&activity.getType()>0){
					double pr=aa.getTraddingPrice()/aa.getPrice();
					if(pr>0.8d){
						rqModel.setStatu(ReturnStatus.ParamError);
						rqModel.setStatusreson("结算价不得高于活动价的80%");
						return rqModel;
					}
				}
	...
	public ReturnModel edit_AActivityProduct(Long weiid, Long actProID, Short state, String reason) {
		...
		if (activityProducts != null) {
			...
			iPDao.set_redis_productAct(activityProducts.getProductId());
		} else {
	...
	public PageResult<AActivity> find_ActivityList(int pageIndex, int pageSize) {
		// 获取活动列表
		String hql = " from AActivity a where a.state=1 order by a.type desc,a.applyEndTime desc";
		...
		if (actPro != null) {
			...
			if (list != null && list.size() > 0) {
				for (AActProducts ss : list) {
					...
					show.setClassId(actPro.getClassId());  
					baseDAO.save(show);
				}
			}
		}
	...
	public PageResult<AActivityProductsResult> find_ActDetail_prolist(String dateTime,int id,Long sellerid, Long actid, Short state, int pageIndex, int pageSize) {
		AActivityTimespans aTimespans = baseDAO.get(AActivityTimespans.class, id);
		Date beginTime = null;
		Date endTime = null;
		StringBuffer endTimesBuffer = new StringBuffer(dateTime);
		if(!dateTime.equals("0")){
			if(aTimespans!=null){
				endTime = new Date();
				beginTime = new Date();
				String beginTimes = dateTime+" "+aTimespans.getBeginTimeStr();
				//beginTimesBuffer.append(" "+aTimespans.getBeginTimeStr());
				endTimesBuffer.append(" "+aTimespans.getEndTimeStr());
				beginTime =DateUtils.format(beginTimes,"yyyy-MM-dd HH:mm:ss");
				endTime =DateUtils.format(endTimesBuffer.toString(),"yyyy-MM-dd HH:mm:ss");
			}
			if(id<0){
				endTime = new Date();
				beginTime = new Date();
				endTimesBuffer.append(" 23:59:59");
				beginTime = DateUtils.format(dateTime, "yyyy-MM-dd");
				endTime = DateUtils.format(endTimesBuffer.toString(), "yyyy-MM-dd HH:mm:ss");
			}
		}
		StringBuilder sb = new StringBuilder();
		...
		if (state != null) {
			switch (state) {
			case 0:// 未开始
				sb.append(" and a.beginTime>:begindate");
				map.put("begindate", beginTime);
				break;
			case 1:// 正在进行
				sb.append(" and a.beginTime<:nowdate and a.endTime>=:nowdate");
				map.put("nowdate", nowtime);
				break;
			case 2:// 已结束
				sb.append(" and a.endTime<:enddate");
	...
	public PageResult<ActProductShowResult> find_AActivityProducts(Integer classId,int pageIndex,int pageSize){	
		String keyName="actproduct828_index_"+pageIndex+"_size_"+pageSize;
		//10分钟缓存
		PageResult<ActProductShowResult> result=(PageResult<ActProductShowResult>)RedisUtil.getObject(keyName);
		if(result!=null&&result.getList()!=null&&result.getList().size()>0){
			return result;
		}
		Integer activiyId=ParseHelper.toInt( AppSettingUtil.getSingleValue("actId"));
		Limit limit=Limit.buildLimit(pageIndex, pageSize);
		//活动产品展示列表
		String keyName_show="proShow828_act_"+activiyId+"_classid_"+classId+"_index_"+pageIndex+"_size_"+pageSize;
		PageResult<AActProductsShowTime> resultPage = (PageResult<AActProductsShowTime>)RedisUtil.getObject(keyName_show); //activityDao.find_AActProductsShowTimes(activiyId, classId,limit);
		if(resultPage==null||result.getList()==null||result.getList().size()<=0){
			resultPage=activityDao.find_AActProductsShowTimes(activiyId, classId,limit);
			if(resultPage!=null&&resultPage.getList()!=null&&resultPage.getList().size()>0){
				RedisUtil.setObject(keyName_show, resultPage,3600); 
			}
		}
		if(resultPage.getList()!=null&&resultPage.getList().size()>0){
			List<Long> pids=new ArrayList<Long>(); 
			List<Long> actpidsList=new ArrayList<Long>();
			for (AActProductsShowTime pp : resultPage.getList()) {
				pids.add(pp.getProductId());
				actpidsList.add(pp.getProActId());
			}
			List<PProducts> prolist=productDao.findProductlistByIds(pids, Short.parseShort(ProductStatusEnum.Showing.toString())); 
			List<AActivityProducts> actproductlist=find_AActivityProductsByIds(actpidsList);
			List<ActProductShowResult> resultlist=new ArrayList<ActProductShowResult>();
			for (AActProductsShowTime aa : resultPage.getList()) {
				ActProductShowResult mo=new ActProductShowResult();
				mo.setActId(aa.getActId());
				mo.setBeginTime(aa.getBeginTime());
				mo.setEndTime(aa.getEndTime());
				mo.setSort(aa.getSort());
				mo.setProActId(aa.getProActId());
				mo.setProductId(aa.getProductId());
				mo.setClassId(aa.getClassId());
				mo.setActPid(aa.getActPid());
				for (PProducts pro : prolist) {
					if(pro.getProductId().longValue()==aa.getProductId().longValue()){
						mo.setProductTitle(pro.getProductTitle());
						mo.setProductImg(ImgDomain.GetFullImgUrl(pro.getDefaultImg(), 75));
						mo.setRetailPrice(pro.getDefaultPrice());
					}
				}
				for (AActivityProducts ac : actproductlist) {
					if(ac.getProductId().longValue()==aa.getProductId().longValue()){
						mo.setActPrice(ac.getPrice()); 
					}
				}
				resultlist.add(mo);
			}
			PageResult<ActProductShowResult> result2= new PageResult<ActProductShowResult>( resultPage.getTotalCount(),limit,resultlist);
			RedisUtil.setObject(keyName, result2,600); 
			return result2;
		}
		return new PageResult<ActProductShowResult>();
	}
}
