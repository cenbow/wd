...
public class DealsService extends BaseService implements IDealsService {
	...
	if (result == null) {
		...
		if(cid>0){
			hql="select a from PProducts a,AHomeProducts b  where a.productId=b.productId and b.homeId in(select homeId from AHomeMain where classId=? and type=? order by position) and a.state=? order by b.sort ";
			page = baseDAO.findPageResult(hql, limit, new Object[] { cid, type,Short.parseShort(ProductStatusEnum.Showing.toString()) });
		}
		if(homeId>0){
			hql = "select a from PProducts a,AHomeProducts b where a.productId=b.productId and b.homeId=? and a.state=? order by b.sort";
