...
public class BasicShopMgtServiceImpl extends BaseService implements IBasicShopMgtService {
	...
	public List<ShopClassVO> getShopClass(Integer parentId, Long weiId) {
		...
		List<PProductClass> ppclass1List=null;
		//判断weiID是否是品牌商ID
		int brandtype=iDagentService.verBrandSupplier(weiId);
		//如果是联合品牌的供应商weiId
		if(brandtype==ParseHelper.toInt(BrandType.manager.toString())){
			ppclass1List = this.getPProductClass((short)1);
			if(ppclass1List!=null&&ppclass1List.size()>0){
				// 转换成List<ShopClassVO>
				for (PProductClass ps : ppclass1List) {
					ShopClassVO vo = new ShopClassVO();
					vo.setClassId(ps.getClassId());
					vo.setClassName(ps.getClassName());
					vo.setLevel(ps.getStep());
					vo.setWeiid(weiId);
					vo.setParetId(ps.getParentId()== null ? 0 : ps.getParentId());
					vo.setSort(ParseHelper.toShort(ps.getSort().toString()));
					vo.setState((short)1);
					result.add(vo);
				}
			}
	...
	public ShopInfoMsg UP_getShopInfo(Long weiNo, Long weiID, String tiket) {
		...
		if (shopInfo != null && shopInfo.getShopName() != null && !"".equals(shopInfo.getShopName())) {
			...
			result.setShopName(basicShopMgtDAO.getNickNameById(user==null?null:user.getWeiId()));
		}
...
//			result.setLogo(ImgDomain.GetFullImgUrl(shopInfo.getShopImg()));
			...
			result.setLogo(basicShopMgtDAO.getImageById(user==null?null:user.getWeiId()));// ImgDomain.GetFullImgUrl(user.getImages())
		}
		if (shopInfo != null && shopInfo.getShopBusContent() != null && !"".equals(shopInfo.getShopBusContent())) {
			...
			String busString = basicShopMgtDAO.getBusContentById(user==null?null:user.getWeiId());
		...
		String sregion = "";// 区域
		if (seller!=null&&seller.getProvince() != null) {
			TRegional t = basicShopMgtDAO.get(TRegional.class, seller.getProvince());
			if (t != null)
				sregion = t.getName();
		}
		if (seller!=null&&seller.getCity() != null) {
