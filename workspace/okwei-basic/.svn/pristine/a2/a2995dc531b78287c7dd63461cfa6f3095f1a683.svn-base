...
public class DAgentMgtDao implements IDAgentMgtDao {
	...
	@Override
	public DAgentApply getDAgentApply(Long weiid,Integer brandid){
		return baseDAO.getNotUniqueResultByHql(" from DAgentApply a where a.weiId=? and a.brandId=?", weiid,brandid);
	}
	...
	public List<DAgentInfo> findDagentInfoByWeiid(Long weiid){
		if(weiid==null||weiid<=0)
			return null;
		return baseDAO.find(" from DAgentInfo a where a.weiId=? ", weiid);
	}
	
	public List<DAgentInfo> find_DAgentInfoBySuperWeiid(Long weiid,Integer brandid){
		if(weiid==null||weiid<=0||brandid==null||brandid<=0)
			return null;
		return baseDAO.find(" from DAgentInfo a where a.superWeiid=? and  a.brandId=?", weiid,brandid);
	}
	...
	public List<DAgentInfo> find_DAgentInfosByBrandId(Integer brandid,Integer level){
		if(brandid==null||brandid<=0)
			return null;
		Map<String, Object> map=new HashMap<String, Object>();
		StringBuilder sb=new StringBuilder();
	...
	public PageResult<DAgentInfo> find_DAgentInfosByBrandId(Integer brandid,Long agentweiid,Integer level,int pageIndex,int pageSize){
		...
		sb.append(" from DAgentInfo a where a.brandId =:brandId ");
		map.put("brandId", brandid);
		if(agentweiid!=null&&agentweiid>0){
			sb.append(" and a.weiId=:weiId ");
			map.put("weiId", agentweiid);
		}
		if(level!=null){
			sb.append(" and a.level=:level ");
			map.put("level", level);
...
//		return baseDAO.count("select sum(a.memberCount) from DAgentTeam a where a.brandId=? ", brandid);
	...
	public Double getAgentCost(Integer brandid, Short grade) {
		Double cost = null;
		DBrandAgentInfo da=baseDAO.get(DBrandAgentInfo.class, brandid);
		if(da!=null){
		if(grade!=null&&grade==1){
			cost=da.getAgencyOne();
		}
		if(grade!=null&&grade==2){
			cost=da.getAgencyTwo();
		}
		if(grade!=null&&grade==3){
			cost=da.getAgencyThree();
		}
		}
		return cost;
	}
	...
	public DAgentApply getDAgent(int brandid, Long weiID) {
		return baseDAO.getNotUniqueResultByHql("from DAgentApply a where a.brandId=? and a.weiId=?", brandid,weiID);
	}

	@Override
	public UWeiSeller getUWeiSeller(Long tuiweiId) {
		return baseDAO.get(UWeiSeller.class, tuiweiId);
	}

	@Override
	public DBrandAgentInfo get_DBrandAgentInfo(Integer brandid) {
		return baseDAO.get(DBrandAgentInfo.class, brandid);
	}
}
