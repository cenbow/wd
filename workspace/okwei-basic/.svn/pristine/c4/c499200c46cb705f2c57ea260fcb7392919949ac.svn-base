		...
		if(page!=null&&page.getList().size()>0){
			...
			if (findNewProduct!=null&&findNewProduct.size()>0) {
				for (AActivityProducts product : findNewProduct) {
					...
					AActShowProducts actProducts=basicShopMgtDAO.get(AActShowProducts.class, product.getProActId());
					if(actProducts!=null){
						sub.setStockCount(actProducts.getStockCount()==null?0:actProducts.getStockCount());//添加活动库存
					}
					list.add(sub);
				}
