...
import com.okwei.cartportal.web.ShopCartMgtController;
...
import com.okwei.service.impl.product.ShopManagerService;
...
public class CartVerFiveService extends BaseService implements ICartVerFiveService {
	...
	@Autowired
	private IBasicShoppingCartMgtService shoppingCartMgtService;
			...
			for (SettlementParam settle : paramlist) {
				if (settle.getProductList() != null && settle.getProductList().size() > 0) {
					for (SettlementProductParam pp : settle.getProductList()) {
						PProductStyles styles= super.getById(PProductStyles.class, pp.getStyleId());
						if(styles!=null){
							pp.setProNum(styles.getProductId()); 
							PProducts products = super.getById(PProducts.class, styles.getProductId());
							if(products.getPublishType()!=null&&1==products.getPublishType()){
								...
								Double price= agentService.getProductPriceByWeiid(weiid, pp.getStyleId());
								pp.setPrice(price);
							} 
				...
				if (settle.getProductList() != null && settle.getProductList().size() > 0) {
					...
					for (SettlementProductParam pp : settle.getProductList()) {
						...
						if (style != null) {
							...
							}else if (settle.getBuyType()!=null&&settle.getBuyType()==Short.parseShort(ShoppingCartTypeEnum.Wholesale.toString())) {
								int pCount=0;
								for (SettlementProductParam tt : settle.getProductList()) {
									if(tt.getProNum().longValue()==pp.getProNum()){
										pCount+=tt.getCount();
									}
								}
								List<PProductBatchPrice> batchlist=shoppingCartMgtService.getBatchPricess(products.getSupplierWeiId(), pp.getProNum(), Short.parseShort(ProductStatusEnum.Showing.toString()));
								Double priceBatch= shoppingCartMgtService.getshoppcartpricebycount(pCount, batchlist);
								if(priceBatch!=null&&priceBatch>0){
									pro.setProductPrice(priceBatch);
								}
							}
							/*----------产品是否在限时抢购中----------------*/
