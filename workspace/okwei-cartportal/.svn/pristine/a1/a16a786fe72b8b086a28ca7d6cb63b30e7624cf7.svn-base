...
<script type="text/javascript" src="http://base2.okwei.com/js/shopcart/common/parsehelp.js"></script>
<title>（购物车）确认订单</title>
<style>
div.jbzl_shdz_yes .mzh_span {
	margin: 0px 14px 0 0;
	float: left;
}
			...
			<div class="tab_marcen bg_white fl">
				<div class="fl nei_guis">
					...
					<c:if test="${storeOrder==1}">
						<div class="fl mzh_width_100 f14 xinxi_fadb61 mb_20">
							您本次的购物金额满足落地店标准，请设置您的落地店地址，以便系统以后可将销售订单按此地址就近分配给您！<span class="fr color_red mr_10 shou" id="close">X</span>
						</div>
					</c:if>
					<div class="jbzl_ns">
						<div class="jbzl_shdz">
							...
							<div>
								<input type="hidden" id="addressId_txt" value="${address.addressId}" />
							</div>
					 ...
					 --%>
								<div class=' ${addr.addressId==address.addressId?"jbzl_shdz_yes":"jbzl_shdz_no"} mb_20' name="divaddress" caddrId="${addr.addressId }">
									...
									默认落地店地址
										...
										<input type="hidden" name="isDefault" value="${addr.isDefault}" /> <input type="hidden" name="isShopAddress" value="${addr.isShopAddress}" />
					...
					<!-- 判断是否是海外购 -->
					<c:if test="${isNeedIdcard==1 }">
					<div class="comment_le fl f12 ft_c3">
						买家身份证号： <input type="text" class="btn_h24" placeholder="海外购产品，需要填写身份证" id="hwgUserIdcard" />
					</div>
					</c:if>
					...
					<form action="/shopCartMgtVerFive/preSubmitNew" method="post">
						<input type="hidden" id="addr_Id" name="addrId" value="${addrId}" /> <input type="hidden" id="listJson" name="listJson" value='${scidJson}' />
					</form>
					<input type="hidden" id="txtfirstOrderFull" value="${firstOrderFull}" /> <input type="hidden" id="totalPrice" value="${totalPrice}" />
					<c:forEach items="${showList}" var="cart" varStatus="cartSta">
						<div class="fl widt_w1156 bor_si">
							...
							<c:if test="${ cart.fristOrderDiffMoney > 0}">
								<div class="fl mzh_width_100 f14 xinxi_fadb61">
									该系列内的产品采购满${cart.fristOrderMoney}元，即可成为该商城的<a href="#" class="ft_lan">落地店</a> 。您需要再购买该系列价值<span class="color_red">￥${cart.fristOrderDiffMoney}元</span>的产品！<span class="ft_red">（加盟即赠现金卷，多买多增哦）</span> <a href="http://www.${cart.supplierId}.<%=okweidomain %>?opType=3&demandId=${cart.demandId}&w=${user.tgWeiID}" target="_blank" class=" f14 mr_10 fr" style="color: #3366cc;">返回系列区采购&gt;&gt;</a>
								</div>
							</c:if>
							<c:forEach items="${cart.productList}" var="product" varStatus="productSta">
								<div name="product_supplier" class="width_marl fl bor_bo">
									...
									<c:if test="${!empty product.activityModel}">
										<div class="fl widt_004 f12 ft_c3">正在参加活动</div>
									</c:if>
									<c:if test="${empty product.activityModel }">
		...
		$(function() {
		  ...
		  $(".jbzl_shdz_yes_div_0").click(function() {
			...
			$("#submitOrder").click(function() {
				...
				var tem= ${isNeedIdcard};
				if(tem==1){
					var idcard=$("#hwgUserIdcard").val();//身份证id
					if(!IDValid(idcard)){
						$("#hwgUserIdcard").focus()
						return;
					}
				...
				$oj.each(function(){
					...
					$(this).parent().find("div[name=product_supplier]").each(function(){
