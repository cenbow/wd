...
$(function() {
	...
	cookieWeiID = getCookie("weiID");
	cookieUserPwd = getCookie("userPwd");
	// 判断cookie中 是否有用户名 和密码
	if (cookieWeiID != null && cookieWeiID != "" && cookieUserPwd != null && cookieUserPwd != "") {
		$("#txtWeiID").val(cookieWeiID);
		$("#txtPwd").val(cookieUserPwd);
		$("#txtWeiID").css({
			color : "#000"
		});
		$("#txtPwd").css({
...
function login() {
	...
	// 值是否发生过改变
	if (weiID == cookieWeiID && cookieUserPwd == pwd) {
		loginType = 2;
	}
	$("#btnLogin").html("登录中...");
	$("#btnLogin").unbind();
		...
		success : function(result) {
			if (result != null) {
				...
				if (result.status == 1) {
					...
					if (domain.indexOf("?") >= 0) {
						location.href = domain + "&tiket=" + result.message;
					}
					else {
						location.href = domain + "?tiket=" + result.message;
					}
