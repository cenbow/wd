...
.mzh_lmbt_one_ck{width: 185px;}
</style>
</head>

<body class="bg_f3">
<input type="hidden" id="pubProductType" value="${pubProductType}" />
		...
		<div style="border: 0px; background: none;" class="fl conter_right">
			...
			<div class="fl conter_right mt_10 gysgl_xzfl bor_si bg_white"
				...
				<div class="gysgl_xzfl_2" id="chainSelete">
					...
					<c:if test="${pubProductType != 5 }">
						<!-- 品牌-->
	                    <div class="gysgl_fl" style="width: 197px;display: none;" id="third_brand">
	                        <div class="fl" style="padding: 5px;">
	                            <input type="text" class="fl" value="test" id="brandTxt"/>
	                            <img src="/statics/images/u2.png" class="fr shou ml_10" id="brandSearch"/>
	                        </div>
	                        <div class="mzh_lmbt_one_ck" style="height: 269px;border-left: none;" id="third_brand_list">
	                        
	                            <!-- <div class="mzh_lmbt_one_ck_no" name="mzh_lmbt_one_ck_brand" >
	                                <span>阿迪达斯</span>
	                                <img src="/statics/pic/jiantou_3_25.png" class="gysgl_fl_img">
	                            </div> -->
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			//选择品牌
			$("#chainSelete").on("click","[name=mzh_lmbt_one_ck_brand]",function(){
				$("[name=mzh_lmbt_one_ck_brand]").attr("class","mzh_lmbt_one_ck_no");
				$(this).attr("class","mzh_lmbt_one_ck_yes");
				$("#path").html("").html('<span class="gysgl_xzds_wz">'+$("#first div.mzh_lmbt_one_ck_yes").attr("namedesc")+'</span> <span class="gysgl_xzds_wz">&gt;</span>'+
						...
						'<span class="gysgl_xzds_wz">'+$("#third_brand div.mzh_lmbt_one_ck_yes").attr("namedesc")+'</span> <span class="gysgl_xzds_wz">&gt;</span>');
			});
			
			/* 选择分类联动 */
			$("#chainSelete").on("click",".mzh_lmbt_one_ck_no",function(){
				var t = $(this);
				if(t.attr("name")=="mzh_lmbt_one_ck_brand"){
					return; 
				}
				var id = t.attr("value");
				var to = t.attr("to");
				...
				if(!(to=='fourth' && $("#pubProductType").val() == 5)){
					var toDom = $("#" + to);
					toDom.show();
			...
			$("#submitBtn").click(function(){
				...
				if($("#pubProductType").val() != 5){
					var brandId = $("[name=mzh_lmbt_one_ck_brand].mzh_lmbt_one_ck_yes").attr("value");
					brandId = (brandId == undefined) ? "" : brandId;
					window.location = "/Product/editProductInfo?operation="+operation+"&&classID="+classId+"&&tempID="+templateId+"&&productId="+productId+"&&brandId="+brandId;
				} else {
		...
		function searchBrand(id,brandName){
			var brandList = $("#third_brand_list");
			$.get(window.basePath+"publishProduct/getBrandByCondition",{"classId":id,"brandName":brandName},function(data){
				brandList.empty();
				if(data.length > 0){
					for(var i = 0; i < data.length; i++){ 
						var	value = data[i].brandId;
						var	name = data[i].brandName;
						var html ="";
						html += '<div class="mzh_lmbt_one_ck_no" '+'namedesc='+name+' value='+ value +' name="mzh_lmbt_one_ck_brand">';
						html += '<span>'+name+'</span>';
						html += '<img src="/statics/pic/jiantou_3_25.png" class="gysgl_fl_img"></div>';
