...
<title>退款详情</title>
<script type="text/javascript" src="<%=request.getContextPath()%>/statics/js/seller/orderdetails.js"></script>
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#agreerefundk").bind("click", function() {
			agreedwin($(this).attr("data"));
		});
		$("#refundsh").bind("click",function(){
			confirmcargowin($(this).attr("data"));
		});
	})
	function confirmsh() {
...
<body>
	<div class="weiz_iam f12 fm_song">
		当前位置：<a href="/maininfo/maininfo">微店中心</a>&gt; <a href="/seller/buylist">订单列表</a>&gt;<span>售后详请</span>
	</div>
	<div class="ddxq_gg">
		<!-- 流程 -->
				...
				<c:when test="${details.refundWay ==1 }">
					...
					<c:choose>
						...
						<c:when test="${details.refundType ==7 }">
							<!-- 交易关闭 -->
							<div class="ddxq_gg_flow_red_5_1_no">
						</c:when>
					</c:choose>
	...
	<div class="ddxq_gg_ddzt">
		<c:choose>
			<c:when test="${details.refundType == 0 }">
				...
				<div id="abefore">
					<div class="ddxq_gg_ddzt_xx">
						当前订单状态： <b>申请退款中</b>； <span class="color_999">若超过48小时未处理，系统会默认同意。请及时处理</span>
					</div>
					...
					<c:choose>
							...
							<div class="ddxq_gg_ddzt_xx_1 f14 ml_30 mt_20">
								<c:choose>
									<c:when test="${details.orderType == 1 }">
										<a href="javascript:void(0);" id="agreerefundk" data="${details.backOrder }" class="wdqb_cz_tx" style="float: none; display: inline-block; color: #333;">同意</a> 或
										<a href="delivery?orderNo=${details.supOrderID }">填写物流信息发货</a>
										<br />
										<span class="ft_sihui">*消费者提交申请后48个小时内，若您在这段时间没有对该申请进行任何操作，则系统会默认同意消费者的退款申请。</span>
									</c:when>
									<c:otherwise>
										<a href="javascript:void(0);" id="agreerefundk" data="${details.backOrder } class=" wdqb_cz_tx" style="float: none; display: inline-block; color: #333;">同意</a>
										<a href="javascript:void(0);" onclick="notagreedwin('${details.backOrder }')" class="wdqb_cz_tx" style="float: none; display: inline-block; margin-left: 20px; color: #333;">不同意</a>
						...
						<c:otherwise>
							<!-- 退货 -->
							<div class="ddxq_gg_ddzt_xx_1 f14 ml_30 mt_20">
								<a href="javascript:void(0);" onclick="showafter()" class="wdqb_cz_tx" style="float: none; display: inline-block; color: #333;">同意</a> <a href="javascript:void(0);" onclick="notagreedwin('${details.backOrder }')" class="wdqb_cz_tx" style="float: none; display: inline-block; margin-left: 20px; color: #333;">不同意</a> <input type="text" placeholder="不同意理由" value="" id="msg" class="ddxq_gg_text ml_20" style="height: 30px; float: none; display: inline-block;" />
							</div>
						</c:otherwise>
					</c:choose>
				</div>
				<div id="after" style="display: none">
					<div class="ddxq_gg_ddzt_xx" style="line-height: 40px; font-size: 16px; margin: 0px;">请填写收货信息</div>
					<jsp:include page="/jsp/selleruserinfo/address.jsp">
						<jsp:param name="list" value="${list }" />
					</jsp:include>
					<div style="width: 1130px; float: left; margin-top: 20px;">
						<a href="javascript:void(0);" onclick="okaddress('${details.backOrder}')" class="wdqb_cz_tx" style="float: none; display: inline-block; color: #333;"> 确认收货地址</a>
					</div>
			...
			<c:when test="${details.refundType == 3 }">
				...
				<div class="ddxq_gg_ddzt_xx_1 mt_10 ml_30">买家退货理由：${details.reason }</div>
				<div class="ddxq_gg_ddzt_xx_1 f14 ml_30 ">
			...
			<c:when test="${details.refundType == 5 }">
				<!-- 买家已发货 -->
				<div class="ddxq_gg_ddzt_xx">
					当前订单状态：<b>采购商已发货</b>； <a href="javascript:void(0);" id="refundsh"  data="${details.backOrder}">确认收货</a>
				</div>
			...
			<c:when test="${details.refundType == 6 }">
				...
				<c:choose>
					<c:when test="${details.refundWay==1 }">
						...
						<div class="ddxq_gg_ddzt_xx_1 ml_30">微店网预计在15个工作日内将退款金额原路退回</div>
					</c:when>
					<c:otherwise>
						<div class="ddxq_gg_ddzt_xx">
							当前订单状态：<b>已确认收货</b>；
								...
								<td class="conter_right_xx_cz_table_55" style="border-bottom: 0px; width: auto;"><c:forEach var="pro" items="${details.proList }" varStatus="status">
										<div class="conter_right_xx_cz_table_55_div" style="${status.index==0?'border: 0px;padding-top:0px;':''}">
											<div class="crxczt5d_0">
												<a href="http://www.${userinfo.weiID }.<%=okweidomain %>/cpxq.html?pNo=${pro.productId}"><img src="${pro.productImg }"></a>
											</div>
	...
	<input id="processId" type="hidden" value="${details.backOrder }" />
	<!-- 确认收货提示 -->
