         ...
         <div class="oneci_ztai fl" style="margin-left:17px;">
            ...
            <div class=" fl f12">
            	...
            	<select class="w150 bor_si" id="ordertype">
                    ...
                    <option value="28" <c:if test="${dt==28}">selected</c:if>>兑换订单</option>
            	</select> 
            </div>
				...
				<div class="conter_right_xx_cz  ${varStr.index==0?'':'mt_20' }" style="border-bottom: 0px;">
					<div class="conter_right_xx_cz_ddh">
						<div class="conter_right_xx_cz_ddh_1">
							...
							<c:if test="${order.orderType==28}">
							<span class="phd_tb">兑换单</span>
							</c:if> 
							<c:if test="${order.orderType==27}">
							<span class="phd_tb">代理单</span>
							</c:if> 
						</div> 
							...
							<tr>
								...
								<c:forEach var="product" items="${order.proList }">
										<div class="conter_right_xx_cz_table_55_div" style="border: 0px; padding-top: 0px;">
											...
											<c:if test="${order.type==0||order.BuyerId!=null}">
											<c:choose>
								...
								<b> <fmt:formatNumber value="${order.totalPrice-order.postPrice}" pattern="0.00"></fmt:formatNumber></b>
								</td>
								<td class="conter_right_xx_cz_table_15xh_fic1" >
									<div class="crxczt5d_sfk">
										<c:choose>  
										   ...
										   <c:otherwise> 
											<b><fmt:formatNumber value="${order.totalPrice}" pattern="0.00"></fmt:formatNumber></b> <span class="col_aaa">(含邮费：${order.postPrice })</span>
										   </c:otherwise>  
										</c:choose> 
									...
									<div class="crxczt5d_cz">
										<c:choose>  
										   ...
										   <c:otherwise> 
											...
											<c:when test="${order.orderState==1}">
												<!-- 已付款 -->
												<a href="delivery?orderNo=${order.orderNo }" style="color: #fff" class="crxczt5d_ljzf">发货</a>
												<a  target="_blank" href="buydetails?orderNo=${order.orderNo }" class="crxczt5d_qxdd">订单详情</a>
											</c:when>
											<c:when test="${order.orderState==2}">
												<!-- 已发货 -->
												<a  target="_blank" href="${mydomain}/order/logistics?orderNo=${order.orderNo }" class="crxczt5d_qxdd">查看物流</a>
												<a  target="_blank" href="buydetails?orderNo=${order.orderNo }" class="crxczt5d_qxdd">订单详情</a>
											</c:when>
											<c:when test="${order.orderState==3}"> 
												<!-- 已收货 -->
