...
$(function() {
	// 验证输入
	$("input[name=firstPrice]").keyup(function() {
		$this = $(this);
		var tmptxt = $this.val();
		...
		var sfk = $("#editTotalPrice").text();
		if (parseFloat(tmptxt) > parseFloat(sfk)) {
			tmptxt = sfk;
		}
		$this.val(tmptxt);
	});
	$("input[name=firstPercent]").keyup(function() {
		$this = $(this);
		var tmptxt = $this.val();
		tmptxt = tmptxt.replace(/[^\d.]/g, ""); // 清除“数字”和“.”以外的字符
		tmptxt = tmptxt.replace(/^\./g, ""); // 验证第一个字符是数字而不是.
	...
	$("[name=payType]").on("click", function() {
		if ($(this).attr("id") != "mzh_quane") {
			$("#weikuan").show();
		}
		else {
			$("#weikuan").hide();
...
function refused(orderNo) {
	$.ajax({
		...
		success : function(data) { 
			if (data.msg == "1") { 
				alert("成功拒绝此铺货单！",true);
				location.href = "buylist?dt=-1&ds=-1";
			}else {
				alert("拒绝失败！");
			}
//			location.reload();
