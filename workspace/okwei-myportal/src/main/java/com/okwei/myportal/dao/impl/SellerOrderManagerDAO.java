...
@Repository
public class SellerOrderManagerDAO extends BaseDAO implements ISellerOrderManagerDAO
{
    ...
    public int getCountOBookAssistlist(Long weiid, Short typestates) {
            if (weiid > 0 && !ObjectUtil.isEmpty(typestates)) {
                    ...
                    Map<String, Object> map = new HashMap<String, Object>();
                    map.put("weiid", weiid);
                    map.put("typeStates", Integer.parseInt(typestates.toString()));
...
//                  return findByMap(hqlString, map);
                    ...
                    String hql="select count(1),o.state from OSupplyerOrder o where  o.supplyerId=:weiid and o.orderType in(:typeStates) and o.state not in(:states) and o.sellerDel!=2 group by o.state";
                    Map<String, Object> map = new HashMap<String, Object>();
                    map.put("weiid", weiid);
                    map.put("typeStates", typeStates);
                    Short[] states = new Short[] { Short.parseShort(OrderStatusEnum.Canceled.toString()), Short.parseShort(OrderStatusEnum.Delete.toString()), Short.parseShort(OrderStatusEnum.Tovoided.toString()) };
