		...
		if (!$(this).hasClass("yes_bgs") && !$(this).hasClass("zero")) {
			...
			if ($("#yearTimeStr").val().length>0) {
				if ($("#monthTimeStr").val().length>0) {
					window.location.href = "platformList?dt=" + datatype+"&yearTimeStr="+ $("#yearTimeStr").val()+"&monthTimeStr="+ $("#monthTimeStr").val();
					return;
				}
				window.location.href = "platformList?dt=" + datatype+"&yearTimeStr="+ $("#yearTimeStr").val();
				return;
			}
			window.location.href = "platformList?dt=" + datatype ;
			return;
			}
	});
...
function onClear(){
	window.location.href = "/platform/platformList";
}
// 获取URL参数
