...
<html>
    ...
    <body>
        ...
        <span class="logo"><a href="javascript:void(0)">卡前置接入</a>
        </span>
        <span class="fr about"><a href="<%=path %>about.jsp">关于</a>
        </span>
        </header>
        <section class="info">
         <form action="<%=path %>toPay.htm" method="post" name="payForm">
        <table border="0" cellspacing="0" cellpadding="0" class="table_ui">
           <tr>
            ...
            <tr>
                <td width="100">
                    <span class="ft_gray">银行卡号：</span>
                </td>
                <td>
                   <input type="text" class="i_text" name="card_no" id="card_no"/>
                    <span id="cardtip"></span>
                </td>
            </tr>
            <tr>
                <td width="200">
                    <span class="ft_gray">已绑定银行卡列表：</span>
                </td>
                <td>
            ...
            <tr>
                <td colspan="2">
                     <input type="button" onclick="paySubmit()" class="btn orange w100 mt10" value="提交去支付" />
                </td>
            </tr>
        </table>
        <input type="hidden" id="user_id" name="user_id" value="<%=request.getParameter("user_id") %>"/>
          <input type="hidden" name="paymod" value="preposit"/>
         <input type="hidden" name="money_order" value="0.01"/>
         <input type="hidden" name="name_goods" value="高级套房一间"/>
        </form>
        </section>
...
function queryUserCard() {
    ...
    ajaxFunction('<%=path %>infoQuery.htm?operate=query_bankcard_list&user_id=' + user_id, 'query_bankcard_list');

}
function queryCardBin(card_no) {
    ajaxFunction('<%=path %>infoQuery.htm?operate=query_card_bin&card_no=' + card_no, 'query_card_bin');
...
function isSelect(){
    ...
    for(var i=0;i<no_agrees.length;i++){
