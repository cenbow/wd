...
public class LLPayUtil{
     ...
     * @return
     */
    public static String GetSubmitHtml(PaymentInfo paymentInfo) {
    	JSONObject jsonObject =JSON.parseObject(JSON.toJSONString(paymentInfo));
    	 // 按照key做首字母升序排列
    	StringBuffer content = new StringBuffer();
    	content.append("<form id='payBillForm' action='" + ServerURLConfig.PAY_URL + "' method='post'>");
        List<String> keys = new ArrayList<String>(jsonObject.keySet());
        for (int i = 0; i < keys.size(); i++)
        {
            String key = (String) keys.get(i);
            ...
            content.append("<input type='hidden' name='" + key + "' value='" + value + "'/>");
        }
        content.append("<input type='submit' value='tijiao' style='display:none;'></form>");
        content.append("<script>document.forms['payBillForm'].submit();</script>");
