...
public class ProductListService extends BaseService implements
	...
	@Override
	public ProductManageVO getManageMsg(LoginUser user,Short productstate,int pagesize,int pageindex,String content) {
		// TODO 自动生成的方法存根
...
//		}
		...
		manageMsg.setYun((short) 0);
		manageMsg.setBatch((short) 0);
		manageMsg.setVerifer((short) 0);
		Short type = user.getWeiType();
		if(type!=null)
		{
			if(BitOperation.getIntegerSomeBit(type, 1)==1)
				manageMsg.setYun((short) 1);
			if(BitOperation.getIntegerSomeBit(type, 2)==1)
				manageMsg.setBatch((short) 1);
			if(BitOperation.getIntegerSomeBit(type, 6)==1)
				manageMsg.setVerifer((short) 1);
		}
		/**
		 * 获取订单个数
		 */
		SupplyOrderCount orderCount = (SupplyOrderCount) RedisUtil.getObject("SellerOrderCount_"+user.getWeiID());
		if(orderCount==null)
		{
			orderCount=new SupplyOrderCount();
			List<Object[]> orderObs = productManageDao.getOrderCount(user.getWeiID());
			Long needpay = 0l;
			Long needsend = 0l;
			Long needreseive = 0l;
			...
			RedisUtil.setObject("SellerOrderCount_"+user.getWeiID(), orderCount, 1800);
		}
		
		/**
		 * 获取产品个数
		 */
		SupplyerProductCount productCount = (SupplyerProductCount) RedisUtil.getObject("ProductCount_"+user.getWeiID());
		if(productCount==null)
		{
			productCount=new SupplyerProductCount();
			List<Object[]> productObs = productManageDao.getProductsCount(user.getWeiID());
			Long saleing = 0l;
			Long saveing = 0l;
			Long verifying = 0l;
			Long fail = 0l;
			...
			RedisUtil.setObject("ProductCount_"+user.getWeiID(), productCount, 1800);
		}
		...
		Long totalCount = productManageDao.getProductCountyByState(user.getWeiID(), productstate,content);
		...
		List<Object[]> objects = productManageDao.getProductsBySupID(user.getWeiID(), productstate, pagesize, pageindex,content);
		int maxsort = productManageDao.getmaxSort(user.getWeiID());
		int minsort = productManageDao.getminSort(user.getWeiID());
