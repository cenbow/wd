...
>>>>>>> .r1250
        ...
        if(!ObjectUtil.isEmpty(tiket))
        {
            ...
            // 设置cookies
            Cookie cookie = new Cookie("tiket2",tiket);
        ...
        if(!ObjectUtil.isEmpty(cookie_tiket) && !"notiket".equals(cookie_tiket))
        {
            ...
            else
            {
                ...
                httpResponse.addCookie(cookie2);
                // 没有找到用户信息，没有登陆，跳转登录页
                httpResponse.sendRedirect("http://port.okwei.com/login.aspx?back=" + domainUrl + "&a=1&c=" + cookie_tiket);
                return;
            }
        }
        else
        {
            ...
            {
                // 没有登陆，跳转登录页
                httpResponse.sendRedirect("http://port.okwei.com/login.aspx?a=2&back=" + domainUrl);
                return;
            }
