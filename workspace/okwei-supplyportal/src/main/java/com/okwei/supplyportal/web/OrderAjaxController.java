...
public class OrderAjaxController extends BaseSSOController {
	...
	switch (key) {
	    case "delivery":// 发货
		MsgResult resultMsgResult = orderService.delivery(orderNo, weiid, kuaidi, kuaidiname, danhao);
	...
	// 退款
	try {
	    TOrderBackTotal refund = orderBase.getEntity(TOrderBackTotal.class, processId);
	...
	MsgResult result = editRefundState(weiId, processId, false, "供应商不同意");
    ...
    // }
    // MsgResult mr =
    // orderService.sendGoods(supplyOrderid,weiid,dcomanpyNo,deliveryCompany,deliveryOrderNo);
    // if(mr.getState() == 1)
    // {
    // return "1";
    // }
