...
public class TesterController {
	...
	private String packageOutPath = "com.egou.common.bean";// 指定实体生成所在包的路径
	private String authorName = "zy";// 作者名字
	...
	// 数据库连接
	private static final String URL = "jdbc:mysql://localhost:3306/easygou?useUnicode=true&characterEncoding=utf-8";
	 ...
	 * 构造函数
	 */
	public void GenEntityOracle(String tname) {
		// 创建连接
		Connection con;
		// 查要生成实体类的表
		...
		try {
			...
			String content = parse(tname, colnames, colTypes, colSizes);

			try {
				File directory = new File("");
				// System.out.println("绝对路径："+directory.getAbsolutePath());
				// System.out.println("相对路径："+directory.getCanonicalPath());
				String path = AppSettingUtil.getSingleValue("imgPathTemp");
				System.out.println(path);
//				System.out.println("java/?/" + path.substring(path.lastIndexOf("/com/", path.length())));
				...
				// ".java";
				String outputPath = path + "/java/" + "/" + initcap(tname) + ".java";
	 ...
	 * @return
	 */
	private String parse(String tname, String[] colnames, String[] colTypes, int[] colSizes) {
	...
	public String init(HttpServletRequest request) {
		GenEntityOracle(request.getParameter("tb"));
		return "sss";
	}
